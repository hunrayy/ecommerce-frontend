...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if(formData.newPassword.length < 6){
        ...
        setFormData((prev) => ({
            ...prev,
            passwordError: 'Passwords do not match!Passwords do not match!'
        }))
    } else {
        setFormData((prev) => ({
            ...prev,
      ...
      const feedback = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/reset-password`, {email: formData.email, newPassword: formData.newPassword}, {
        headers: {
      ...
      if(feedback.data.code == 'error'){
        ...
        setServerSuccess(<p>Password reset successful, you can now <Link style={{color: "purple", textDecoration: "none"}} to='/login'>Login</Link></p>);
      }
